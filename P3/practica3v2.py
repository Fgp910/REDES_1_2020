...
'''
    ...
    Salida:
        -sec: lista con los segundos.
        -bps: bits por segundo de cada segundos.

    Descripción: Esta función recibe la salida de tshark (con el comando -qz
    io,stat,<filtro>).
...
def tsharkToSerie(salida):
    ...
    for linea in salida:
        fila = linea.split('|')
        if len(fila) != 6 or "Interval" in linea:
            pass
        else: #Desempacamos las filas de la salida del comando (conociendo su formato)
            sec.append(int(fila[1].split('<>')[0]) + 1) #Empieza en 1
            bps.append(int(fila[3])*8) #Bytes a bits
...
if __name__ == "__main__":
    ...
    codigo,salida = ejecutarComandoObtenerSalida("tshark -r {0} -qz io,stat,1,'eth.src eq {1}'".format(args.tracefile, args.mac))
    if codigo:
        sys.exit(-1)

    sec, bps = tsharkToSerie(salida)
    pintarSerieTemporal(sec,
                        bps,
    ...
    codigo,salida = ejecutarComandoObtenerSalida("tshark -r {0} -qz io,stat,1,'eth.dst eq {1}'".format(args.tracefile, args.mac))
    if codigo:
        sys.exit(-1)
