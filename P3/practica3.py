...
if __name__ == "__main__":
    ...
    codigo,salida = ejecutarComandoObtenerSalida("tshark -r {} -T fields -e frame.time_delta_displayed -Y 'ip.src eq {} and tcp and (not icmp)'".format(args.tracefile, args.ip_flujo_tcp))
    if codigo:
        sys.exit(-1)
    ...
    codigo,salida = ejecutarComandoObtenerSalida("tshark -r {} -T fields -e frame.time_delta_displayed -Y 'ip.dst eq {} and tcp and (not icmp)'".format(args.tracefile, args.ip_flujo_tcp))
    if codigo:
        sys.exit(-1)
